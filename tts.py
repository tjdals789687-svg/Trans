...
def synthesize(text: str, language: str = "en") -> Dict[str, Optional[str]]:
    ...
    if not text:
        return {"success": False, "audio_path": None, "audio_url": None, "error": "Text is empty"}

    TTS = _safe_import_tts()
    if TTS is None:
        err = "TTS is disabled" if not TTS_ENABLED else "TTS library not installed"
        return {"success": False, "audio_path": None, "audio_url": None, "error": err}

    model_name = TTS_MODELS.get(language, TTS_MODELS.get("en"))
    if model_name is None:
        return {"success": False, "audio_path": None, "audio_url": None, "error": f"No TTS model for '{language}'"}
    ...
    try:
        ...
        return {"success": False, "audio_path": None, "audio_url": None, "error": f"TTS failed: {e}"}

    audio_path = f"{AUDIO_DIR}/{filename}".replace("\\", "/")
    audio_url = f"/static/audio/{filename}"

    return {"success": True, "audio_path": audio_path, "audio_url": audio_url, "error": None}
